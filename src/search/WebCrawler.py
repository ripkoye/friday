...
from search.Seeds import Seeds
import asyncio
...
class WebCrawler:
    def __init__(self, queries: list[str], keywords: list[str] = None):
        """
        ...
        Args:
            ...
            keywords (list[str]): List of keywords to filter results.
        """
        print(f"DEBUG: WebCrawler init with queries: {queries}")
        seed = Seeds(queries)
        self.urls = seed.urls
        ...
        elif len(self.urls) < 5:
            print(f"WARNING: Only got {len(self.urls)} URLs, expected more")
